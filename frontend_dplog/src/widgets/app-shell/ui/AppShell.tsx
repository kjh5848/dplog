      ...
      <div className="flex h-screen w-full bg-white dark:bg-[#050505] text-slate-900 dark:text-white overflow-hidden">
        ...
        >
          ...
          <nav className="flex-1 space-y-2">
            <SidebarItem icon={Store} label="내 가게" href="/dashboard/stores/new" active={pathname.startsWith('/dashboard/stores')} isCollapsed={isCollapsed} />
            <SidebarItem icon={BarChart3} label="순위 조회" href="/dashboard/ranking" active={pathname.startsWith('/dashboard/ranking')} isCollapsed={isCollapsed} />
            <SidebarItem icon={Sparkles} label="황금 키워드 발굴" href="/dashboard/keywords" active={pathname.startsWith('/dashboard/keywords')} isCollapsed={isCollapsed} />
            <SidebarItem icon={Search} label="실시간 조회" href="/dashboard/realtime" active={pathname.startsWith('/dashboard/realtime')} isCollapsed={isCollapsed} />
            <SidebarItem icon={FileText} label="내 가게 진단 (준비중)" href="/dashboard/report" active={pathname.startsWith('/dashboard/report')} isCollapsed={isCollapsed} />
          </nav>

          <div className="pt-6 border-t border-slate-200 dark:border-white/10 space-y-2 flex flex-col">
            <SidebarItem icon={Settings} label="설정" href="/settings" active={pathname === '/settings'} isCollapsed={isCollapsed} />
            <SidebarItem icon={HelpCircle} label="도움말" href="/help" active={pathname === '/help'} isCollapsed={isCollapsed} />
          ...
          <header className="h-20 border-b border-slate-200 dark:border-white/10 flex items-center justify-between px-8 backdrop-blur-md bg-white/30 dark:bg-black/10 z-20">
            ...
            <div className="flex items-center gap-4">
              <button className="relative size-10 flex items-center justify-center rounded-full hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">
                ...
                <span className="absolute top-2 right-2 size-2 bg-red-500 rounded-full border-2 border-white dark:border-black" />
              </button>
