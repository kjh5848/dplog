...
import { RankingChartControls } from './RankingChartControls';
import { RankingChartViewChart } from './RankingChartViewChart';
import { RankingChartViewList } from './RankingChartViewList';
import { RankingChartViewCards } from './RankingChartViewCards';
...
export default function RankingChartClient(props: RankingChartProps) {
  ...
  const { fullData, keywords } = useMemo(() => {
    ...
    keywordEntries.forEach((chart) => {
      chart.dailyRanks.forEach((dr) => {
        ...
        if ((dr as any).checkedTime) {
          entry.checkedTime = (dr as any).checkedTime;
        }
  ...
  return (
    <div className="bg-white dark:bg-white/[0.02] rounded-2xl border border-slate-200 dark:border-white/[0.06] p-6 mb-32">
      <RankingChartControls
        ...
        chartInterval={props.chartInterval}
        setChartInterval={props.setChartInterval}
      />
