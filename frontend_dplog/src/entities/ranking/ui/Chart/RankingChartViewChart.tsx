...
export const ChartTooltip = ({ active, payload, label }: any) => {
  ...
  const metrics = payload.filter((p: any) => ['방문자리뷰', '블로그리뷰'].includes(p.name));
  const rankings = payload.filter((p: any) => !['방문자리뷰', '블로그리뷰'].includes(p.name));
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl p-4 text-sm min-w-[220px]">
      <p className="font-bold text-slate-700 dark:text-slate-300 mb-3 text-xs border-b border-slate-100 dark:border-slate-700 pb-2">
        {payload?.[0]?.payload?.checkedTime || (label ? new Date(label).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric', weekday: 'short' }) : '')}
      </p>
    ...
    return (
      <ResponsiveContainer width="100%" height={360}>
        <ComposedChart data={formattedData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
          ...
          <YAxis yAxisId="left" reversed type="number" domain={[1, (dataMax: number) => Math.max(dataMax, 5)]} allowDecimals={false} tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}위`} width={40} />
          {showReviews && (
            <YAxis yAxisId="right" orientation="right" type="number" allowDecimals={false} tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}`} width={40} />
          )}
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-100 dark:border-slate-700">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <ComposedChart data={formattedData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            ...
            <YAxis reversed type="number" domain={[1, (dataMax: number) => Math.max(dataMax, 5)]} allowDecimals={false} tick={{ fontSize: 10, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}위`} />
            <Tooltip content={<ChartTooltip />} />
            {keywords.map((kw, idx) => visibleKeywords.has(kw) && (
