...
const BlogWritersModal = ({ show, handleClose, onSave, editData }) => {
    ...
    useEffect(() => {
        ...
        if (editData) {
            ...
            setValue("nplaceRewardBlogWritersType", editData.nplaceRewardBlogWritersType);
    ...
    return (
        <Modal show={show} onHide={handleClose}>
            <Modal.Header closeButton>
                <Modal.Title>블로그 기자단 {editData ? "수정" : "추가"}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Form onSubmit={handleSubmit(onSubmit)}>
                    <Form.Group className="mb-3">
                        <Form.Label>상품</Form.Label>
                        <Form.Select {...register("nplaceRewardBlogWritersType", { required: "상품을 선택하세요" })} disabled={!!editData}>
                            {products.map((product, index) => (
                                <option key={index} value={product.name}>{product.value}</option>
                            ))}
                        </Form.Select>
                        {errors.nplaceRewardBlogWritersType && <p className="text-danger">{errors.nplaceRewardBlogWritersType.message}</p>}
                    </Form.Group>
                    <Form.Group className="mb-3">
...
export default BlogWritersModal;