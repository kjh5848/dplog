...
public class NomadscrapNplaceRankTrackChartRepository {
    ...
    public ResNomadscrapNplaceRankPostTrackChartDTO postTrackChart(ReqNomadscrapNplaceRankPostTrackChartDTO reqDto) {
        final UriComponents uriComponents = UriComponentsBuilder
                .fromHttpUrl("http://%s/v1/nplace/rank/track/chart".formatted(nomadscrapServerIp))
                .queryParam("apiKey", nomadscrapServerApiKey)
                .build();
        ResponseEntity<ResNomadscrapNplaceRankPostTrackChartDTO> responseEntity = UtilFunction.getWebClientAutoRedirect("json")
                .post()
                .uri(uriComponents.toUriString())
                .bodyValue(reqDto)
                .exchangeToMono(clientResponse -> clientResponse.toEntity(ResNomadscrapNplaceRankPostTrackChartDTO.class))
                .block();
