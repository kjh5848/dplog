...
빌링키 발급 API 응답에서는 `billingKeyInfo.billingKey`처럼 중첩된 구조로 결과가 내려옵니다. 서버에서 PortOne 응답을 사용할 때는 이 필드를 참고해 빌링키를 추출하세요.
...
> PortOne API 호출은 `PortOneV2Client#createBillingReservation` 을 통해 수행되며, 실패 응답 전문(`rawResponse`)은 서버 로그에서 확인할 수 있습니다.
