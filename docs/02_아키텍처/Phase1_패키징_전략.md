...
- 패키징 스크립트는 OS별로 분리합니다.
  - macOS: `./build_edge_mac.sh` → `backend_python/dist/macos/main.app`
  - Windows: `.\build_edge_windows.ps1` → `backend_python\dist\windows\D-PLOG.exe`
- 빠른 실행 검증은 `./test_desktop.sh`와 `.\test_desktop_windows.ps1`로 수행합니다.
